...
const HomeQuote: React.FC<Props> = ({ categories }) => {
    ...
    const category = () => {
        const filteredCategories = categories.filter(cat => cat.id === params.categoryId);
        return filteredCategories.length > 0 ? filteredCategories[0].category : "All";
    };

    let filteredQuotes;

    if (params.categoryId) {
        filteredQuotes = quotes.filter(quote => quote.category === params.categoryId);
    } else {
